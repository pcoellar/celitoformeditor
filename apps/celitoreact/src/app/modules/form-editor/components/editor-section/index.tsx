...
const EditorSection = (props: IProps) => {
    const [text, setText] = useState<string|undefined>(props.section.title);
    ...
    return (
        <S.RootContainer>
            <S.Title>
                <div ref={divRef} contentEditable={true} onInput={(e) => handleOnchange(e, props.section.id)} style={{outline: 'none',border: 0}}>{text}</div>
            </S.Title>
            <S.IconContainer><Icon iconName="Trash" onClick={() => handleIconOnClick(props.section.id)} style={{cursor:"pointer"}} /></S.IconContainer>
        </S.RootContainer>
    )
}
